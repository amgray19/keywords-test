...
// === SUGGESTED ALTERNATIVES APPENDED TO SENTENCES ===

// Load suggestions into a lookup table
...
// Helper: create styled span with suggestions
...
// After the output is generated, scan and inject suggestions
const outputEl = document.getElementById("output");
if (outputEl) {
  const observer = new MutationObserver(() => {
    document.querySelectorAll("#output p, #output li").forEach(el => {
      if (el.dataset.suggestionAdded === "true") return;

      const foundTerms = Object.keys(keywordSuggestions).filter(term =>
        el.textContent.toLowerCase().includes(term)
      );

      if (foundTerms.length > 0) {
        // Only show the first match
        const suggestion = createSuggestionSpan(foundTerms[0]);
        if (suggestion) {
